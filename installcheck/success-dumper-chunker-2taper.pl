...
is($sorted_usage_by_tape[1]->{'nb'}, '1' , 'one dle on tape 1');
is($sorted_usage_by_tape[1]->{'nc'}, '1' , 'one part on tape 1');
is($sorted_usage_by_tape[1]->{'tape_label'}, 'STO-2-00001' , 'label tape_label on tape 1');
is($sorted_usage_by_tape[1]->{'size'}, '1050' , 'size 1050  on tape 1');
ok(!exists $reply->{'body'}->[0]->{'report'}->{'usage_by_tape'}->[2], 'use two tape');
...
is($reply->{'body'}->[0]->{'report'}->{'summary'}->[1]->{'dle_status'}, 'nodump-FLUSH', 'dle_status is correct') || diag("summary: " . Data::Dumper::Dumper($reply->{'body'}->[0]->{'report'}->{'summary'}));
ok(!exists $reply->{'body'}->[0]->{'report'}->{'summary'}->[2], 'Only two summary');
...
taped
  ...
  storage-2     :   1      1050k      1050k (100.00%) (100.00%)
