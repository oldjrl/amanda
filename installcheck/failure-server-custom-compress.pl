...
localhost diskname2 $diskname {
    ...
    server-custom-compress "$cwd/amcat-error"
}
EODLE
$testconf->write();
...
FAILED DUMP DETAILS:
  ...
  sendbackup: info end
  ? data compress: amcat-error: failure X
...
localhost:diskname2 $timestamp 0        64k dump failed: [write to compression program failed: Broken pipe], written (00:00:00)
