...
$testconf->add_param('debug-auth', '9');
$testconf->add_param('debug-dumper', '9');
my $cwd = getcwd();
...
my $post_data = '';
if ($Amanda::Constants::FAILURE_CODE && rand(2) > 0) {
...
localhost:diskname2 $timestamp 0        64k dump failed: [write to encryption program failed: Broken pipe], written (00:00:00)
