...
my $post_data = '';
if (rand(2) > 1) {
    $post_data = '{"FAILURE":{"XFER_DEST_HOLDING_PORT_WRITE_DATA":"0"}}';
} else {
    $post_data = '{"FAILURE":{"XFER_DEST_HOLDING_PORT_WRITE_DATA":"0","DISABLE_NETWORK_SHM":"1"}}';
}
...
is_deeply($reply->{'body'}->[0]->{'report'}->{'failure_summary'}, [
        "  localhost diskname2 lev 0  FAILED [Failed to write data to holding file '$Installcheck::TMP/holding/$timestamp/localhost.diskname2.0.tmp': No space left on device]",
...
dumped          :   1      1050k       525k (100.00%) (100.00%)
wait for writing
wait to flush
writing to tape
dumping to tape
