...
localhost diskname2 $diskname {
    ...
    client-custom-compress "$cwd/amcat-error"
}
EODLE
$testconf->write();
...
#is($reply->{'body'}->[0]->{'report'}->{'notes'}->[3], '  taper: tape TESTCONF01 kb 32 fm 1 [OK]' , 'notes[3] is correct');
#ok(!exists $reply->{'body'}->[0]->{'report'}->{'notes'}->[4], 'no notes[4]');
#ok(!exists $reply->{'body'}->[0]->{'report'}->{'notes'}->[3], 'no notes[3]');
#is($reply->{'body'}->[0]->{'report'}->{'failure_summary'}->[0], '  localhost diskname2 lev 0  FAILED [amcat-error: failure X]' , 'failure_summary[0] is correct');
#is_deeply($reply->{'body'}->[0]->{'report'}->{'failure_summary'}, [
#        "  localhost diskname2 lev 0  FAILED [amcat-error: failure X]",
##        "  localhost diskname2 lev 0  FAILED [shm_ring cancelled]",
#        "  localhost diskname2 lev 0  FAILED [amcat-error: failure X]",
...
$reply->{'body'}->[0]->{'status'}->{'dles'}->{'localhost'}->{'diskname2'}->{$timestamp}->{'storage'}->{'TESTCONF'}->{'taper_time'} = undef if defined $reply->{'body'}->[0]->{'status'}->{'dles'}->{'localhost'}->{'diskname2'}->{$timestamp}->{'storage'}->{'TESTCONF'}->{'taper_time'};
...
is_deeply($reply->{'body'}->[0]->{'status'}->{'stat'},
	...
	'taped' => {
		...
		'storage' => {
			'TESTCONF' => {
				'estimated_stat' => '12.1904761904762',
				'real_size' => '65536',
				'nb' => '1',
				'real_stat' => '12.1904761904762',
				'estimated_size' => '537600'
			}
		},
	},
...
dumping to tape
failed to tape
taped           :   1        64k       525k (100.00%) (100.00%)
