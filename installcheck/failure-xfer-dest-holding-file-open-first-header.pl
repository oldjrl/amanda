...
my $post_data = '';
if (rand(2) > 1) {
    $post_data = '{"FAILURE":{"XFER_DEST_HOLDING_PORT_OPEN_HEADER":"0"}}';
} else {
    $post_data = '{"FAILURE":{"XFER_DEST_HOLDING_PORT_OPEN_HEADER":"0","DISABLE_NETWORK_SHM":"1"}}';
}
...
is_deeply($reply->{'body'}->[0]->{'report'}->{'failure_summary'}, [
        "  localhost diskname2 lev 0  FAILED [Failed to open holding file '$Installcheck::TMP/holding/$timestamp/localhost.diskname2.0.tmp': No space left on device]",
